...
function proxiedMediaUrl(abs, typeHint = '') {
  return '/api?url=' + encodeURIComponent(abs) + (typeHint ? '&media_type=' + encodeURIComponent(typeHint) : '');
}

function absolutizeM3u8Url(value, playlistUrl) {
  const raw = String(value || '').trim();
  if (!raw) return raw;
  try { return new URL(raw, playlistUrl).toString(); } catch (_) { return raw; }
}

function rewriteM3u8(body, url) {
  // Mobile Safari is stricter than desktop hls.js. Rewrite every playable URI,
  // including nested playlists, encryption keys, EXT-X-MAP init segments, and
  // normal TS/fMP4 segments. Leaving just one external URI can make iPhone
  // keep loading for a long time then fail.
  return String(body || '').replace(/\r\n/g, '\n').split('\n').map(line => {
    let out = line;
    const t = line.trim();
    if (!t) return line;

    // Attribute form: #EXT-X-KEY:METHOD=AES-128,URI="..."
    // Also covers #EXT-X-MAP:URI="..." and other URI attributes.
    out = out.replace(/URI=("([^"]+)"|'([^']+)'|([^,\s]+))/gi, (m, quoted, dval, sval, uval) => {
      const val = dval || sval || uval || '';
      if (!val || val.startsWith('data:') || val.startsWith('blob:')) return m;
      const abs = absolutizeM3u8Url(val, url);
      return 'URI="' + proxiedMediaUrl(abs) + '"';
    });

    // Plain media / child playlist line. Keep comments as comments after
    // rewriting any URI attributes above.
    if (!t.startsWith('#')) {
      const abs = absolutizeM3u8Url(t, url);
      const isPlaylist = /\.m3u8(\?|#|$)/i.test(abs);
      out = proxiedMediaUrl(abs, isPlaylist ? 'hls' : '');
    }
    return out;
  }).join('\n');
}
  ...
  if (q.url) {
    ...
    try {
      ...
      if (isM3u8) {
        ...
        res.setHeader('Access-Control-Allow-Origin','*');
        res.setHeader('Cache-Control','public, max-age=3, s-maxage=10');
